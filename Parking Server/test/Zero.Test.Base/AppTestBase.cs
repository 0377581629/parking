...
using Zero.Test.Base.TestData;

namespace Zero.Test.Base
{
    /// <summary>
    /// This is base class for all our test classes.
    /// It prepares ABP system, modules and a fake, in-memory database.
    /// Seeds database with initial data.
    /// Provides methods to easily work with <see cref="ZeroDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(ZeroDbContext context)
            {
        ...
        protected TResult UsingDbContext<TResult>(Func<ZeroDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<ZeroDbContext, Task<TResult>> func)
        {
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<ZeroDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<ZeroDbContext, Task<TResult>> func)
        {
            TResult result;
