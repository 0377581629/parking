...
namespace DPS.Park.Application.Services.Card
{
    ...
    public class CardAppService: ZeroAppServiceBase,ICardAppService
    {
        ...
        public CardAppService(IRepository<Core.Card.Card> cardRepository)
        {
        ...
        private IQueryable<CardDto> CardQuery(QueryInput queryInput)
        {
            ...
            var query = from obj in _cardRepository.GetAll()
                    ...
                    .WhereIf(input != null && !string.IsNullOrWhiteSpace(input.Filter), e => e.Code.Contains(input.Filter) || e.CardNumber.Contains(input.Filter) || e.Note.Contains(input.Filter))
        ...
        protected virtual async Task Update(CreateOrEditCardDto input)
        {
            ...
            {
                var obj = await _cardRepository.FirstOrDefaultAsync(o => o.TenantId == AbpSession.TenantId && o.Id == input.Id);
