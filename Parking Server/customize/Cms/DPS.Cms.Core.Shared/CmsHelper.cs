...
namespace DPS.Cms.Core.Shared
{
    public static class CmsHelper
    {
        ...
        {
            var salaryComponentTypeScope = (CmsEnums.PageLayout)status;
        ...
        {
            return (from status in (CmsEnums.PageLayout[])Enum.GetValues(typeof(CmsEnums.PageLayout))
        ...
        {
            var salaryComponentTypeScope = (CmsEnums.WidgetContentType)status;
            return lang.GetString(salaryComponentTypeScope.GetStringValue());
        }

        public static List<SelectListItem> ListWidgetContentType(int currentLayout, ILocalizationSource lang)
        {
            return (from status in (CmsEnums.WidgetContentType[])Enum.GetValues(typeof(CmsEnums.WidgetContentType))
                select new SelectListItem(lang.GetString(status.GetStringValue()), ((int)status).ToString(), currentLayout == (int)status)).ToList();
        }
